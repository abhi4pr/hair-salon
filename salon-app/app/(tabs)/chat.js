...
export default function Chat() {
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      ...
      >
        <Image
          source={{ uri: item.salon?.images?.[0]?.url || 'https://via.placeholder.com/50' }}
          style={styles.avatar}
        />
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background, paddingTop: insets.top }]}>
      <View style={[styles.header, { borderBottomColor: theme.border }]}>
        <Text style={[styles.headerTitle, { color: theme.textPrimary }]}>{t('chat')}</Text>
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState
              ...
              subtitle="Book a salon appointment to start chatting"
            />
          }
