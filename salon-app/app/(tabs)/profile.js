...
const MENU_ITEMS = [
  ...
  { key: 'paymentMethods', icon: 'card-outline', route: '/profile/payment-methods' },
  { key: 'saved', icon: 'heart-outline', route: '/profile/favorites' },
  { key: 'loyaltyPointsTitle', icon: 'gift-outline', route: '/profile/loyalty' },
  { key: 'settings', icon: 'settings-outline', route: '/profile/settings' },
  { key: 'transactions', icon: 'receipt-outline', route: '/profile/transactions' },
  { key: 'helpCenter', icon: 'help-circle-outline', route: '/help/index' },
  { key: 'privacyPolicy', icon: 'shield-checkmark-outline', route: '/help/privacy' },
];
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.menuCard, { backgroundColor: theme.card, shadowColor: theme.shadow }]}>
          {MENU_ITEMS.map((item, index) => (
            ...
            >
              ...
              <Text style={[styles.menuLabel, { color: theme.textPrimary }]}>{t(item.key)}</Text>
        ...
        <View style={[styles.menuCard, { backgroundColor: theme.card, shadowColor: theme.shadow, marginTop: SPACING.sm }]}>
          <TouchableOpacity style={styles.menuItem} onPress={toggleTheme} activeOpacity={0.7}>
            ...
            <Text style={[styles.menuLabel, { color: theme.textPrimary }]}>{t('darkMode')}</Text>
        ...
        >
          ...
          <Text style={[styles.menuLabel, { color: theme.error }]}>{t('logout')}</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  statsRow: { flexDirection: 'row', marginHorizontal: SPACING.md, borderRadius: RADIUS.xl, padding: SPACING.md, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },
