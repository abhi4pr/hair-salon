...
import {
  View, Text, TouchableOpacity, StyleSheet, ScrollView, KeyboardAvoidingView, Platform, Image,
} from 'react-native';
    ...
    >
      ...
      >
        <View style={styles.logoWrap}>
          ...
          <Text style={[styles.appName, { color: COLORS.primary }]}>SalonApp</Text>
        </View>

        <View style={[styles.card, { backgroundColor: theme.card, shadowColor: theme.shadow }]}>
          <Text style={[styles.title, { color: theme.textPrimary }]}>{t('welcomeBack')}</Text>
          <Text style={[styles.subtitle, { color: theme.textSecondary }]}>{t('signInToContinue')}</Text>
          ...
          <Input
            ...
            leftIcon={<Ionicons name="lock-closed-outline" size={18} color={theme.icon} />}
          />
        ...
        <View style={styles.signupRow}>
          ...
          <TouchableOpacity onPress={() => router.push('/(auth)/register')}>
            <Text style={[styles.signupLink, { color: COLORS.primary }]}>{t('signUp')}</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </KeyboardAvoidingView>
...
const styles = StyleSheet.create({
  ...
  logoCircle: { width: 88, height: 88, borderRadius: 44, alignItems: 'center', justifyContent: 'center', marginBottom: SPACING.sm },
  appName: { fontSize: FONT_SIZE.xxl, fontWeight: '800', letterSpacing: 1 },
  card: { borderRadius: RADIUS.xl, padding: SPACING.lg, shadowOpacity: 0.08, shadowRadius: 16, shadowOffset: { width: 0, height: 4 }, elevation: 4 },
